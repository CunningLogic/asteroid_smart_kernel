...
static irqreturn_t omap_mcbsp_tx_irq_handler(int irq, void *dev_id)
{
	...
	if (irqst_spcr2 & XSYNC_ERR) {
		...
		MCBSP_WRITE(mcbsp_tx, SPCR2, MCBSP_READ(mcbsp_tx, SPCR2) & ~XSYNC_ERR);
	} else {
...
static irqreturn_t omap_mcbsp_rx_irq_handler(int irq, void *dev_id)
{
	...
	if (irqst_spcr1 & RSYNC_ERR) {
		...
		MCBSP_WRITE(mcbsp_rx, SPCR1, MCBSP_READ(mcbsp_rx, SPCR1) & ~RSYNC_ERR);
	} else {
...
void omap_mcbsp_config(unsigned int id, const struct omap_mcbsp_reg_cfg *config)
{
	...
	MCBSP_WRITE(mcbsp, RCERA, config->rcera);
	MCBSP_WRITE(mcbsp, XCERA, config->xcera);
...
static int __devinit omap_mcbsp_probe(struct platform_device *pdev)
{
    ...
    mcbsp->dma_op_mode = pdata->dma_op_mode;
	mcbsp_ptr[id] = mcbsp;
