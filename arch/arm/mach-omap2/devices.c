...
#define MAX_OMAP_MCSPI_HWMOD_NAME_LEN		16

void __init omap2_init_mcspi(struct omap2_mcspi_platform_config *pdata, int spi_num)
{
	struct omap_hwmod *oh;
	struct omap_device *od;
	char *name = "omap2_mcspi";
	char oh_name[MAX_OMAP_MCSPI_HWMOD_NAME_LEN];
	int l;

	l = snprintf(oh_name, MAX_OMAP_MCSPI_HWMOD_NAME_LEN,
		     "mcspi%d", spi_num);
	WARN(l >= MAX_OMAP_MCSPI_HWMOD_NAME_LEN,
	     "String buffer overflow in MSCPI%d device setup\n", spi_num);
