...
int omap_voltage_scale(struct voltagedomain *voltdm)
{
	...
	if( !vnom ){
		/*
		*  This value should have been initialized in omap3_vdd_data_configure but some time
		*  it failed when changing opp frequency in board specific (see and struct opp_frequencies
		*  and call to opp_find_freq_exact for more details) event if this initialization is made
		*  with a call to device_initcall.
		*/

		//Init again
		vdd->nominal_volt = vnom = get_init_voltage(&vdd->voltdm);
...
static int __init omap_voltage_init(void)
{
	...
	if (!(cpu_is_omap34xx() || cpu_is_omap44xx()) || 1) {
