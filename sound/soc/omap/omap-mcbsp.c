...
static int mcbsp_reg_open_file(struct inode *inode, struct file *file)
{
	file->private_data = inode->i_private;
	return 0;
}

static ssize_t mcbsp_reg_read_file(struct file *file, char __user *user_buf,
			       size_t count, loff_t *ppos)
{
	ssize_t ret = 0;
	struct omap_mcbsp_data *mcbsp_data = file->private_data;
	...
	ret+= sprintf(buf + ret,
		"%s:  0x%04x\n%s:  0x%04x \
		 \n%s:  0x%04x\n%s:  0x%04x \
		 \n%s:  0x%04x\n%s:  0x%04x\n",
		"MCBSPLP_SPCR2_REG", regs->spcr2,
		"MCBSPLP_SPCR1_REG", regs->spcr1,
		"MCBSPLP_RCR2_REG", regs->rcr2,
		"MCBSPLP_RCR1_REG", regs->rcr1,
		"MCBSPLP_XCR2_REG", regs->xcr2,
		"MCBSPLP_XCR1_REG", regs->xcr1);
	...
	ret += sprintf(buf + ret,
		"%s:  0x%04x\n%s:  0x%04x   \
		 ...
		 \n%s:  0x%04x\n%s:  0x%04x \
		 \n%s:  0x%04x\n",
		"MCBSPLP_SRGR2_REG", regs->srgr2,
		"MCBSPLP_SRGR1_REG", regs->srgr1,
		"MCBSPLP_MCR2_REG", regs->mcr2,
		"MCBSPLP_MCR1_REG", regs->mcr1,
		"MCBSPLP_RCERA_REG", regs->rcera,
		"MCBSPLP_XCERA_REG", regs->xcera,
		"MCBSPLP_PCR_REG", regs->pcr0);
	...
	ret += sprintf(buf + ret,
		"%s:  0x%04x\n%s:  0x%04x   \
		 ...
		 \n%s:  0x%04x\n%s:  0x%04x \
		 \n%s:  0x%04x\n%s:  0x%04x\n",
		"MCBSPLP_RCERC_REG", regs->rcerc,
		"MCBSPLP_RCERD_REG", regs->rcerd,
		"MCBSPLP_XCERC_REG", regs->xcerc,
		"MCBSPLP_XCERD_REG", regs->xcerd,
		"MCBSPLP_RCERE_REG", regs->rcere,
		"MCBSPLP_RCERF_REG", regs->rcerf,
		"MCBSPLP_XCERE_REG", regs->xcere,
		"MCBSPLP_RCERF_REG", regs->xcerf);

	ret += sprintf(buf + ret,
		"%s:  0x%04x\n%s:  0x%04x   \
		 \n%s:  0x%04x\n%s:  0x%04x \
		 \n%s:  0x%04x\n%s:  0x%04x\n",
		"MCBSPLP_RCERG_REG", regs->rcerg,
		"MCBSPLP_RCERH_REG", regs->rcerh,
		"MCBSPLP_XCERG_REG", regs->xcerg,
		"MCBSPLP_XCERH_REG", regs->xcerh,
		"MCBSPLP_XCCR_REG", regs->xccr,
		"MCBSPLP_RCCR_REG", regs->rccr);


	//MCBSPLP_RCERB_REG : not available
	//MCBSPLP_XCERB_REG : not available
	//MCBSPLP_REV_REG : not available
	//MCBSPLP_RINTCLR_REG : not available
	//MCBSPLP_XINTCLR_REG : not available
	//MCBSPLP_ROVFLCLR_REG : not available
	//MCBSPLP_SYSCONFIG_REG : not available
	//MCBSPLP_THRSH2_REG : not available
	//MCBSPLP_THRSH1_REG : not available
	//MCBSPLP_IRQSTATUS_REG : not available
	//MCBSPLP_IRQENABLE_REG : not available
	//MCBSPLP_WAKEUPEN_REG : not available
	//MCBSPLP_XBUFFSTAT_REG : not available
	//MCBSPLP_RBUFFSTAT_REG : not available
	//MCBSPLP_SSELCR_REG : not available
	//MCBSPLP_STATUS_REG : not available
...
static const struct file_operations mcbsp_reg_fops = {
	.open = mcbsp_reg_open_file,
	.read = mcbsp_reg_read_file,
};
...
#if defined(CONFIG_DEBUG_FS)
 	if (!mcbsp_data->debugfs_mcbsp_root) {
		mcbsp_data->debugfs_mcbsp_root = debugfs_create_dir(cpu_dai->name,
						       debugfs_root);
		if (!mcbsp_data->debugfs_mcbsp_root) {
			printk(KERN_WARNING "MCBSP: Failed to create MCBSP debugfs subdirectory\n");
			return err;
		}

		if (!mcbsp_data->debugfs_mcbsp_reg){
			mcbsp_data->debugfs_mcbsp_reg = debugfs_create_file("mcbscp_reg", 0444,
								 mcbsp_data->debugfs_mcbsp_root,
								 mcbsp_data,
								 &mcbsp_reg_fops);

			if (!mcbsp_data->debugfs_mcbsp_reg)
				printk(KERN_WARNING "MCBSP: Failed to create MCBSP register debugfs file\n");
		}
 	}
#endif
...
#if defined(CONFIG_DEBUG_FS)
	if (!mcbsp_data->debugfs_mcbsp_root)
		debugfs_remove_recursive(mcbsp_data->debugfs_mcbsp_root);
#endif
}
				    ...
				    struct snd_soc_dai *cpu_dai)
{
    ...
    /*
     * FC6100 specifi patch:
     * If the format is 16bits and the codec forces us to use 64 bit_clocks,
     * the format won't be correctly supported by the OMAP.
     * Workaround, do the following configuration:
     * - configure 1PHASE I2S link.
     * - 4 words per frame (16bits data; 16bits of 0; 16bits data; 16bits 0).
     * - activate channels 0 and 2, to retrieve only the 2*16bits of valid data.
     * We use the same configuration trick for both direction, playback and capture.
     */
	if (((format == SND_SOC_DAIFMT_I2S) || (format == SND_SOC_DAIFMT_I2S_1PHASE)))
	{
        /* receive */
        /* 4 slots, 1 phase */
        regs->rcr2	&= ~(RPHASE);
        regs->rcr1	&= 0xFFFF80FF;
        regs->rcr1	|= RFRLEN1(3);
        regs->mcr1	|= 0x201;

        /* activate only channel#0 & channel#2 */
        regs->rcera	= 0x5;

        /* transmit */
        /* 4 slots */
        regs->xcr2	&= ~(XPHASE);
        regs->xcr1	&= 0xFFFF80FF;
        regs->xcr1	|= XFRLEN1(3);

        /* activate same as receive */
        regs->mcr2	|= 0x3;
        regs->xcera	= 0x5;
    }

	/* Set FS period and length in terms of bit clock periods don't care when using external clock*/
	switch (format) {
				       ...
				       int clk_id)
{
	...
	/*
	* see Figure 21-26 in "21.4.2.3 Clocking and Framing Data"
	* see Figure 21-39 in "21.4.3 McBSP SRG"
	* For SCLKME see bit 7 in Table 21-76. MCBSPLP_PCR_REG
	*/
	regs->pcr0	&= ~SCLKME;
	regs->pcr0	|=CLKXM;
	regs->pcr0	|=CLKRM;
	...
	if (clk_id == OMAP_MCBSP_SYSCLK_CLKS_FCLK){
		...
		/*
		* For CLKSM see bit 13 in Table 21-60. MCBSPLP_SRGR2_REG
		* For GSYNC see bit 15 in Table 21-60. MCBSPLP_SRGR2_REG
		* CLKSM=0 and SCLKME=0: SRG clock derived from the CLKS pin.
		*/
		regs->srgr2	&= ~CLKSM;
	}
...
static struct snd_soc_dai_driver omap_mcbsp_dai =
{
	...
	.playback = {
		...
		.formats = SNDRV_PCM_FMTBIT_U8 | SNDRV_PCM_FMTBIT_S8 | SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S32_LE,
	},
	.capture = {
		.channels_min = 1,
		.channels_max = 16,
...
#if defined(CONFIG_DEBUG_FS)
	if(!debugfs_root){
		debugfs_root = debugfs_create_dir("omap-mcbsp", NULL);
		if (IS_ERR(debugfs_root) || !debugfs_root) {
			printk(KERN_WARNING
			       "MCBSP: Failed to create debugfs directory\n");
			debugfs_root = NULL;
		}else
			printk(KERN_WARNING
		       		"MCBSP: Succeed to create debugfs directory\n");
	}
#endif
