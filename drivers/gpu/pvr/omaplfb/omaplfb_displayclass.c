...
#ifndef CONFIG_SGX_XRES_REDUCE_PIXELS
#define CONFIG_SGX_XRES_REDUCE_PIXELS 0
#endif

u32 sgx_reduce_pixels_left = CONFIG_SGX_XRES_REDUCE_PIXELS;
u32 sgx_reduce_pixels_right = 0;
u32 sgx_reduce_pixels_down = 0;

module_param(sgx_reduce_pixels_left, uint, S_IRUGO);
MODULE_PARM_DESC(sgx_reduce_pixels_left,
	"Reduce Android's size on framebuffer. Left");
EXPORT_SYMBOL_GPL(sgx_reduce_pixels_left);

module_param(sgx_reduce_pixels_right, uint, S_IRUGO);
MODULE_PARM_DESC(sgx_reduce_pixels_right,
	"Reduce Android's size on framebuffer. Right");
EXPORT_SYMBOL_GPL(sgx_reduce_pixels_right);

module_param(sgx_reduce_pixels_down, uint, S_IRUGO);
MODULE_PARM_DESC(sgx_reduce_pixels_down,
	"Reduce Android's size on framebuffer. Down");
EXPORT_SYMBOL_GPL(sgx_reduce_pixels_down);
...
static OMAP_ERROR InitDev(OMAPLFB_DEVINFO *psDevInfo, int fb_idx)
{
	...
	psPVRFBInfo->sCPUVAddr = psLINFBInfo->screen_base + sgx_reduce_pixels_left * (psLINFBInfo->var.bits_per_pixel / 8);
	psPVRFBInfo->ulWidth = psLINFBInfo->var.xres - sgx_reduce_pixels_left - sgx_reduce_pixels_right;
	psPVRFBInfo->ulHeight = psLINFBInfo->var.yres;
	psPVRFBInfo->ulByteStride = psLINFBInfo->fix.line_length;
	psPVRFBInfo->ulFBSize = FBSize;
	psPVRFBInfo->ulBufferSize =
		psPVRFBInfo->ulHeight * psPVRFBInfo->ulByteStride;
	psPVRFBInfo->ulHeight -= sgx_reduce_pixels_down;
