				 ...
				 struct mxt_message *message)
{
	...
	return mxt_read_reg(data->client, reg,
...
static int mxt_check_reg_init_pdata(struct mxt_data *data, int version)
{
	struct i2c_client *client = data->client;
	...
	if (version == 0x10) {
		config = pdata->config_1;
		config_length = pdata->config_1_length;
		config_crc = pdata->config_1_crc;
	}
	else if (version == 0x20) {
		config = pdata->config_2;
		config_length = pdata->config_2_length;
		config_crc = pdata->config_2_crc;
	  }
	else {
		dev_err(dev, "atmel mxt init : version should be 0x10 or 0x20 instead of %08X\n", version);
		return 1;
	}
...
retry_probe:
	...
	error = mxt_check_reg_init_pdata(data, info->version & 0xF0);
	if (error) {
		/* No onboard pdata infos, try to load config file */
		dev_err(&client->dev, "No onboard config, try to load firmware.\n");
...
static int mxt_initialize_input_device(struct mxt_data *data)
{
	...
	if (data->state == APPMODE) {
		...
		if (ret) {
			dev_err(dev, "Failed to make high CHG\n");
