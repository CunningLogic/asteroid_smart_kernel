                   ...
                   IN HostCmd_DS_COMMAND * resp, IN mlan_ioctl_req * pioctl_buf)
{
    ...
    if (pioctl_buf) {
        ...
        case SET_GET_BF_PERIODICITY:
            ...
            break;
        case TX_BF_FOR_PEER_ENBL:
            txbf->no_of_peers = wlan_le32_to_cpu(*(t_u8 *) & txbfcfg->body);
            offset += sizeof(t_u8);
            for (i = 0; i < txbf->no_of_peers; i++) {
                memcpy(pmadapter, txbf->body.tx_bf_peer[i].peer_mac,
                       (t_u8 *) & txbfcfg->body + offset, MLAN_MAC_ADDR_LENGTH);
                offset += MLAN_MAC_ADDR_LENGTH;
                /* Skip the reserved bytes */
                offset += 2;
                txbf->body.tx_bf_peer[i].bf_enbl =
                    *((t_u8 *) & txbfcfg->body + offset);
                offset += sizeof(t_u8);
        ...
        case SET_SNR_THR_PEER:
            ...
            for (i = 0; i < txbf->no_of_peers; i++) {
                memcpy(pmadapter, txbf->body.bf_snr[i].peer_mac,
                       (t_u8 *) & txbfcfg->body + offset, MLAN_MAC_ADDR_LENGTH);
                offset += MLAN_MAC_ADDR_LENGTH;
                txbf->body.bf_snr[i].snr = *((t_u8 *) & txbfcfg->body + offset);
                offset += sizeof(t_u8);
            }
            break;
        default:
