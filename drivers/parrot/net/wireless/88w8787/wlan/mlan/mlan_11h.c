...
static const
    IEEEtypes_SupportChan_Subband_t wlan_11h_JPN_lower_band = { 34, 4 };
...
wlan_11h_is_dfs_slave(mlan_private * priv)
{
    ENTER();
    LEAVE();
    return !wlan_11h_is_dfs_master(priv);
}
...
wlan_11h_check_chan_report(mlan_private * priv, t_u8 chan)
{
    ...
    if ((!pstate_dfs->dfs_check_pending) &&
        ...
        if (pstate_dfs->dfs_radar_found) {
            PRINTM(MMSG, "Radar was detected on channel %d - prevent"
                   " start interface.\n", chan);
            ret = MLAN_STATUS_FAILURE;
        }
    } else {
...
wlan_11h_radar_detected_handling(mlan_adapter * pmadapter)
{
    ...
    case RDH_STOP_INTFS:
        ...
        /* issues one cmd (DEAUTH/BSS_STOP) to each intf */
        if ((++pstate_rdh->priv_curr_idx) < pstate_rdh->priv_list_count) {
            pmpriv = pstate_rdh->priv_list[pstate_rdh->priv_curr_idx];
#ifdef UAP_SUPPORT
            if (GET_BSS_ROLE(pmpriv) == MLAN_BSS_ROLE_UAP) {
    ...
    case RDH_SET_NEW_CHANNEL:
        ...
        /* can only set new channel for UAP intfs */
        while ((++pstate_rdh->priv_curr_idx) < pstate_rdh->priv_list_count) {
            pmpriv = pstate_rdh->priv_list[pstate_rdh->priv_curr_idx];
#ifdef UAP_SUPPORT
            if (GET_BSS_ROLE(pmpriv) == MLAN_BSS_ROLE_UAP) {
    ...
    case RDH_RESTART_INTFS:
        ...
        while ((++pstate_rdh->priv_curr_idx) < pstate_rdh->priv_list_count) {
            ...
            if (GET_BSS_ROLE(pmpriv) == MLAN_BSS_ROLE_STA &&
                    wlan_11h_is_dfs_master(pmpriv)) {
                // TODO: Ad-hoc create network
                // break; //leads to exit case
            }
            /* NOTE: DON'T reconnect slave STA intfs - infra/adhoc_joiner Do we 
               want to return to same AP/network (on radar channel)? If want to 
               connect back, depend on either: 1. driver's reassoc thread 2.
               wpa_supplicant, or other user-space app */
        }
