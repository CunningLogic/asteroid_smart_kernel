...
#define KSZ8851SNL_LOWSPEED	0//31250000
...
static int ksz8851snl_hw_check(struct net_device *dev)
{
	...
	/* Read the read OBCR register */
	spi_read_hword(priv, REG_BUS_CLOCK_CTRL, &rocbr);
	dev_err(&priv->spi->dev, "Register OCBR 0x%4.4x\n", rocbr);
...
static void ksz8851snl_setrx_work_handler(struct work_struct *work)
{
	...
	if (dev->flags & IFF_PROMISC) {
		...
		spi_write_hword(priv, REG_RX_CTRL1, RX_CTRL_MODE_PROMISC);
			...
			(netdev_mc_count(dev) > FILTER_LIMIT)) {
...
static const struct net_device_ops ksz8851snl_netdev_ops = {
	.ndo_open		= ksz8851snl_net_open,
	.ndo_stop		= ksz8851snl_net_close,
	.ndo_start_xmit		= ksz8851snl_send_packet,
	.ndo_set_multicast_list	= ksz8851snl_set_multicast_list,
	.ndo_tx_timeout		= ksz8851snl_tx_timeout,
   .ndo_get_stats       = ksz8851snl_get_stats,
...
static int __devinit ksz8851snl_probe(struct spi_device *spi)
{
	...
	ret = request_irq(spi->irq, ksz8851snl_irq, IRQF_TRIGGER_FALLING,
			  DRV_NAME, priv);
	if (ret < 0) {
	...
	dev->irq = spi->irq;
	dev->netdev_ops = &ksz8851snl_netdev_ops;
