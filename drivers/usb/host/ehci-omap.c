...
#ifdef CONFIG_EHCI_OMAP_ULPI_SYSFS
	u32 current_reg;
#endif
};
					  ...
					  char *buf)
{
	...
	addr = (omap->current_reg & 0x3F );

	/* Soft reset the PHY using PHY reset command over ULPI */
	if (omap->port_mode[0] == EHCI_HCD_OMAP_MODE_PHY) {
		reg = (addr << EHCI_INSNREG05_ULPI_REGADD_SHIFT) /* interface reg  */
	...
	if (omap->port_mode[2] == EHCI_HCD_OMAP_MODE_PHY) {
		reg = (addr << EHCI_INSNREG05_ULPI_REGADD_SHIFT) /* interface reg  */
		  | (3 << EHCI_INSNREG05_ULPI_OPSEL_SHIFT) /* READ */
		  | (3 << EHCI_INSNREG05_ULPI_PORTSEL_SHIFT) /* PORT3 */
		  | (1 << EHCI_INSNREG05_ULPI_CONTROL_SHIFT); /* START */
		ehci_omap_writel(omap->ehci_base, EHCI_INSNREG05_ULPI, reg);
	}

	while( ( val = ehci_omap_readl(omap->ehci_base, EHCI_INSNREG05_ULPI)) &
	       (1ul << EHCI_INSNREG05_ULPI_CONTROL_SHIFT) )
		udelay(2);
					...
					const char *buf, size_t size)
{
	...
	if (omap->port_mode[0] == EHCI_HCD_OMAP_MODE_PHY) {
		reg = val
		  | (addr << EHCI_INSNREG05_ULPI_REGADD_SHIFT) /* interface reg  */
