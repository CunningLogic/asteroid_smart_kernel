...
#define CCDC_MIN_HEIGHT	 2

#define MAX_CCDC_SBL_WAIT 1000
...
{
	struct isp_device *isp = to_isp_device(ccdc);
	struct isp_res_device *res = v4l2_get_subdevdata(&ccdc->subdev);
	...
	*/
	if (pdata->bt656)
		isp_reg_writel(isp,
			...
			((format->height / 4) << ISPCCDC_VDINT_1_SHIFT),
	...
	/* BT656 */
	if (pdata->bt656) {
		isp_reg_writel(isp, (DIV_ROUND_UP(res->crop.active.top, 2) <<
				     ISPCCDC_VERT_START_SLV0_SHIFT) |
				    (DIV_ROUND_UP(res->crop.active.top, 2) <<
				     ISPCCDC_VERT_START_SLV1_SHIFT),
			       OMAP3_ISP_IOMEM_CCDC, ISPCCDC_VERT_START);
	} else {
		isp_reg_writel(isp, (0 << ISPCCDC_VERT_START_SLV0_SHIFT) |
				    (0 << ISPCCDC_VERT_START_SLV1_SHIFT),
			       OMAP3_ISP_IOMEM_CCDC, ISPCCDC_VERT_START);
	}
			      ...
			      unsigned int max_wait)
{
	...
	if (max_wait == 0)
		max_wait = MAX_CCDC_SBL_WAIT; /* 1 ms */
...
static void ccdc_vd0_isr(struct isp_ccdc_device *ccdc)
{
	...
	if (ccdc->output & CCDC_OUTPUT_MEMORY) {
		/* BT656 buffer completion */
		if (ccdc_input_is_bt656(ccdc)) {
			/* NTSC & PAL have different starting frame (odd/even)... */
			if (format->height == 480)
				tmp_fld = ccdc->bt656_fldstat;
			else
				tmp_fld = !ccdc->bt656_fldstat;

			if ((tmp_fld && ccdc->bt656_fldprev != ccdc->bt656_fldstat) ||
				(ccdc->stopping & CCDC_STOP_REQUEST))
...
static void ccdc_vd1_isr(struct isp_ccdc_device *ccdc)
{
	...
	if (ccdc_input_is_bt656(ccdc)) {
		/* Remember previous FLDSTAT value */
		ccdc->bt656_fldprev = ccdc->bt656_fldstat;
		ccdc->bt656_fldstat =
			!!(isp_reg_readl(isp, OMAP3_ISP_IOMEM_CCDC,
				ISPCCDC_SYN_MODE) & ISPCCDC_SYN_MODE_FLDSTAT);
		return;
	}
...
/*
 * ccdc_s_crop - handle set crop subdev operation
 * @sd : pointer to v4l2 subdev structure
 * @pad : subdev pad
 ...
 */
static int ccdc_s_crop(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh,
		       struct v4l2_subdev_crop *crop)
{
	...
	/* Only source pad has crop capability */
	if (crop->pad != CCDC_PAD_SINK)
		return -EINVAL;

	src_fmt = ccdc->formats[CCDC_PAD_SINK];
	clamp_t(int, crop->rect.left, 0, src_fmt.width - CCDC_MIN_WIDTH);
	clamp_t(int, crop->rect.width, CCDC_MIN_WIDTH,
		src_fmt.width - crop->rect.left);

	clamp_t(int, crop->rect.top, 0, src_fmt.height - CCDC_MIN_HEIGHT);
	clamp_t(int, crop->rect.height, CCDC_MIN_HEIGHT,
		src_fmt.height - crop->rect.top);

	dev_dbg(isp->dev, "%s: W=%d H=%d cL=%d,cT=%d,cW=%d,cH=%d,which=%d\n",
		__func__, src_fmt.width, src_fmt.height, crop->rect.left,
...
/*
 * ccdc_g_crop - handle get crop subdev operation
 * @sd : pointer to v4l2 subdev structure
 * @pad : subdev pad
 * @crop : pointer to crop structure
 * @which : active or try format
 * return zero
 */
static int ccdc_g_crop(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh,
			  struct v4l2_subdev_crop *crop)
{
	struct isp_ccdc_device *ccdc = v4l2_get_subdevdata(sd);
	struct isp_res_device *res = v4l2_get_subdevdata(sd);
	...
	switch (crop->which) {
		case V4L2_SUBDEV_FORMAT_TRY:
			crop->rect = res->crop.request;
		break;
		case V4L2_SUBDEV_FORMAT_ACTIVE:
			crop->rect = res->crop.active;
		break;
		default:
			crop->rect.left = 0;
			crop->rect.width = ccdc->formats[CCDC_PAD_SINK].width;
			crop->rect.top = 0;
			crop->rect.height = ccdc->formats[CCDC_PAD_SINK].height;
	}
...
static const struct v4l2_subdev_pad_ops ccdc_v4l2_pad_ops = {
	...
	.set_crop = ccdc_s_crop,
	.get_crop = ccdc_g_crop
};
