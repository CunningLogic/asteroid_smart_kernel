...
static int tvp5150_s_crop(struct v4l2_subdev *sd,
							struct v4l2_subdev_fh *fh,
							struct v4l2_subdev_crop *a)
{
	struct v4l2_rect rect = a->rect;
	struct tvp5150 *decoder = to_tvp5150(sd);
	v4l2_std_id std;
	int hmax;
	...
	if (a->which == V4L2_SUBDEV_FORMAT_TRY)
	{
		*v4l2_subdev_get_try_crop(fh, a->pad) = a->rect;
		return 0;
	}
...
static int tvp5150_g_crop(	struct v4l2_subdev *sd,
							struct v4l2_subdev_fh *fh,
							struct v4l2_subdev_crop *a)
{
	...
	switch (a->which) {
		case V4L2_SUBDEV_FORMAT_TRY:
			a->rect = *v4l2_subdev_get_try_crop(fh, a->pad);
		break;
...
static int tvp5151_registered(struct v4l2_subdev *sd)
{
	struct tvp5150 *tvp = to_tvp5150(sd);
	int rval;
	u8 msb_id, lsb_id, msb_rom, lsb_rom;
	struct v4l2_ctrl_ops tvp5150_ctrl = {
		.s_ctrl = tvp5150_s_ctrl,
	};

	rval = tvp->pdata->s_power(&tvp->sd, 1);
	if (rval) {
		rval = -ENODEV;
		goto out;
	}
	...
	v4l2_ctrl_handler_init(&tvp->hdl, 4);
	v4l2_ctrl_new_std(&tvp->hdl, &tvp5150_ctrl,
	...
	sd->ctrl_handler = &tvp->hdl;
	if (tvp->hdl.error) {
		int err = tvp->hdl.error;

		v4l2_ctrl_handler_free(&tvp->hdl);
		kfree(tvp);
		return err;
	}
	v4l2_ctrl_handler_setup(&tvp->hdl);

	if (debug > 1)
		tvp5150_log_status(sd);
	tvp5150_write(sd, TVP5150_AUTOSW_MSK, 0xE3); // Unmask NTSC and PAL standards.

	/* Default is no cropping */
	tvp->rect.top = 0;
	if (tvp5150_read_std(sd) & V4L2_STD_525_60)
		tvp->rect.height = TVP5150_V_MAX_525_60;
	else
		tvp->rect.height = TVP5150_V_MAX_OTHERS;
	tvp->rect.left = 0;
	tvp->rect.width = TVP5150_H_MAX;


	rval = tvp->pdata->s_power(&tvp->sd, 0);
out:
	return rval;
}

static int tvp5150_s_power(struct v4l2_subdev *subdev, int enable)
{
	struct i2c_client *client = v4l2_get_subdevdata(subdev);
	struct tvp5150_platform_data *tvp_pdata = client->dev.platform_data;

	int rval;
	v4l2_dbg(1, debug, subdev,"tvp5150_s_power : %d\n",enable);

	if (enable) {
		rval = tvp_pdata->s_power(subdev, 1);
		if (rval)
			goto out;

	} else {
		rval = tvp_pdata->s_power(subdev, 0);
		if (rval)
			goto out;
	}
...
static int tvp5150_open(struct v4l2_subdev *subdev, struct v4l2_subdev_fh *fh)
{
	...
	int rval;
	v4l2_dbg(1, debug, subdev, "tvp5150: open\n");

	rval = decoder->pdata->s_power(&decoder->sd, 1);
	if(rval)
		return rval;
...
{
	struct tvp5150 *decoder = to_tvp5150(subdev);
	v4l2_dbg(1, debug, subdev, "tvp5150: close\n");
	return decoder->pdata->s_power(&decoder->sd, 0);
}
				   ...
				   struct v4l2_subdev_frame_size_enum *fse)
{
	...
	/* Calculate height based on current standard */
	if (decoder->std == V4L2_STD_ALL)
		std = tvp5150_read_std(subdev);
	else
		std = decoder->std;

	if (std & V4L2_STD_525_60)
		fse->min_height = TVP5150_V_MAX_525_60;
	else
		fse->min_height = TVP5150_V_MAX_OTHERS;


	fse->min_width = TVP5150_H_MAX;
...
static struct v4l2_subdev_pad_ops tvp5150_pad_ops = {
	...
	.set_crop =  tvp5150_s_crop,
	.get_crop =  tvp5150_g_crop,
};
			 ...
			 const struct i2c_device_id *id)
{
	...
	core->std = V4L2_STD_ALL;	/* Default is autodetect */
