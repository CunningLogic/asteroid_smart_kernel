...
irqreturn_t mxt224e_interrupt(int, void*);
...
static void dib0700_forward_it_frontend_release(struct dvb_frontend *fe)
{
	...
	dib0700_usb_urb_kill(&adap->stream);
...
void do_mxt224_interrupt(struct work_struct *arg)
{
	...
	if (client)
		mxt224e_interrupt(0, i2c_get_clientdata(client));
}
...
static int dib0700_forward_it_frontend_attach(struct dvb_usb_adapter *adap)
{
	...
	adap->fe = kzalloc(sizeof(struct dvb_frontend), GFP_KERNEL);
	if (adap->fe == NULL)
		return -ENODEV;
	memcpy(&adap->fe->ops, &dib0700_forward_it_frontend_ops,
	...
	adap->stream.complete  = dib0700_forward_it_data_complete;
	...
	request_module(I2C_MODULE_PREFIX "%s", fc6100_i2c_bus2_info->type);
	queue_work(st->wq, &st->work);
	/* submit URBs */
	dib0700_usb_urb_submit(&adap->stream);
...
struct dvb_usb_device_properties dib0700_devices[] = {
	{
		...
		.caps              = DVB_USB_IS_AN_I2C_ADAPTER,
		.usb_ctrl          = DEVICE_SPECIFIC,
		.firmware          = "dvb-usb-dib0700-1.20.forward_it.fw",
		.download_firmware = dib0700_download_firmware,
		.no_reconnect      = 1,
		.size_of_priv      = sizeof(struct dib0700_state),
		.i2c_algo          = &dib0700_i2c_algo,
