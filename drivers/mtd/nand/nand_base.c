...
struct reduced_onfi nand_timing[] =
{
	{
		/* onfi mode 0 edo=0 */
...
struct {
	...
	{0x98, 0xaa, 4}, /* TC58NYG2S3EBAI5 ALS, CLS, DS = 12 instead of 10 */
};
	...
	if (!type->name || !type->pagesize) {
		...
		if (chip->read_byte(mtd) == 'O' &&
			...
			if (chip->onfi_version) {
				...
				chip->chipsize = le32_to_cpu(p->blocks_per_lun) * mtd->erasesize * p->lun_count;
				busw = 0;
...
ident_done:
	...
	printk(KERN_INFO "NAND device: Manufacturer ID:"
		" 0x%02x, Chip ID: 0x%02x %dMiB (%s %s)\n", *maf_id, *dev_id,
