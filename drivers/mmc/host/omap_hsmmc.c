...
static int omap_hsmmc_reg_get(struct omap_hsmmc_host *host)
{
	...
	if (IS_ERR(reg)) {
		...
		*/
		...
		/* Delay needed by some MMC models */
		mdelay(100);
	}
...
static void set_sd_bus_power(struct omap_hsmmc_host *host)
{
	...
	OMAP_HSMMC_WRITE(host, HCTL,
			 OMAP_HSMMC_READ(host, HCTL) | SDBP | IWE);
...
static void omap_hsmmc_enable_sdio_irq(struct mmc_host *mmc, int enable)
{
	...
	OMAP_HSMMC_WRITE(host, CON, con);
	OMAP_HSMMC_WRITE(host, IE, ie);
	OMAP_HSMMC_READ(host, IE); /* flush posted write */
	if (!enable)
...
static void omap_hsmmc_conf_bus_power(struct omap_hsmmc_host *host)
{
	...
	OMAP_HSMMC_WRITE(host, HCTL, value | hctl | IWE);
	...
	OMAP_HSMMC_WRITE(host, SYSCONFIG, value | AUTOIDLE | ENAWAKEUP);
...
static int __init omap_hsmmc_probe(struct platform_device *pdev)
{
	...
	OMAP_HSMMC_WRITE(host, ISE, INT_EN_MASK | CIRQ);
	OMAP_HSMMC_WRITE(host, IE, INT_EN_MASK);
