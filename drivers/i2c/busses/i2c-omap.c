...
static void
omap_i2c_speed_configure(struct omap_i2c_dev *dev,
			struct omap_i2c_bus_platform_data *pdata,
			unsigned long speed) {
	unsigned long internal_clk;
	dev->speed = speed;
...
omap_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	int oldspeed = 0;
	if (msgs[0].addr == 0x11 && dev->speed > 10) {
		oldspeed = dev->speed;
		omap_i2c_speed_configure(dev, pdata, 10);
	}
	omap_i2c_unidle(dev);
	enable_irq(dev->irq);
...
out:
	...
	if (oldspeed && msgs[0].addr == 0x11) {
		omap_i2c_speed_configure(dev, pdata, oldspeed);
	}
	omap_i2c_idle(dev);
